...
@app.route('/api/user/<id>/wishlits/remove', methods=('GET','POST'))
def remove_wish(id):
    profile = user_profile.query.filter_by(userid=id).first()
    #user_wishlist = user_wishlist.query.filter_by(wishid=id).first()
    if request.method=='POST':
        wish_query=db.session.query(user_wishlist).filter_by(user_id=id).first()
        db.session.delete(wish_query)
        db.session.commit()
        return render_template('user_profile.html')
    return render_template('remove_wish.html',userid=id)
...
def wish_share(id):
        ...
        if request.method == 'POST':
            ...
            subject = "My Wishlist"
            wishlist_link = "http://info3180-wishlist-sk3tchk.c9users.io/api/user/"+ id +"/wishlists"
            msg = "Please click on the following link below to view the items on my wishlist: " + " " + wishlist_link +"."+"\n"+"\n"+ "Kind regards," + "\n" + from_name
            message = """From: {} <{}>
To: {} <{}>
Subject: {}
